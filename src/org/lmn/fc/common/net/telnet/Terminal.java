...
public class Terminal extends Canvas implements ClassConnection
    ...
    static final Integer ACTION_DISCONNECT = new Integer(0);
    static final Integer ACTION_CONNECT = new Integer(1);
...
// initialize
    ...
    public Dimension preferredSize()
        ...
        System.out.println("Preferred dim: " + dim);
        return dim;
        */
        }
    ...
    public void drawRow(Graphics g, int row)
        ...
        if (cursorVisible && row == curRow)
            {
            if (DEBUG)
                System.out.println("Drawing cursor: row=" + curRow +
                                   ", col=" + curCol);
    ...
    public void screenRedraw(Graphics g, int top, int bot)
        ...
        for (int i = top; i <= bot; i++)
            {
            if (DEBUG)
                System.out.println("Drawing: " + i + ", curRow: " +
                                   curRow);
            drawRow(g, i);
/*
      g.setColor(getBackground());
...
*/
    ...
    public void screenClean(Graphics g)
        {
        if (DEBUG)
            System.out.println("Cleaning screen: " + dirtyTop +
                               " to " + dirtyBottom);
    ...
    public void write(char[] d, int off, int len, int flags)
        {
        if (DEBUG)
            {
        ...
        if (len == 1)
            ...
            while (len > 0)
                ...
                boolean do_redraw;
                if ((flags & OUTF_RAW) != 0)
                    do_redraw = put_char(d[off]);
                else
                    do_redraw = put_std_char(d[off]);
                if (do_redraw)
                    {
                    if (last_row < curRow)
                        screenDirty(last_row, curRow);
                    else
                        screenDirty(curRow, last_row);
                    }
                off++;
                len--;
                }
    ...
    public void write(char c, int flags)
        {
...
//
    public boolean handleEvent(Event e)
        ...
        if (e.id == e.KEY_ACTION)
            {
            if (emulator != null) emulator.handleEvent(e);
            return true;
            }
