...
public final class VerticalTextIcon implements Icon,
    ...
    private static final String sDrawsInTopRight =
            "\u3041\u3043\u3045\u3047\u3049\u3063\u3083\u3085\u3087\u308E" + // hiragana
     ...
     * @param component
     */
    public VerticalTextIcon(final Component component,
                            final String label)
        {
        this(component,
             label,
    ...
    public void setLabel(final String label)
        {
        fLabel = label;
        fRotation = verifyRotation(label,
                                   fRotation); // Make sure the current rotation is still legal
        recalcDimensions();
        }
    ...
    private void calcDimensions()
        ...
        if (fRotation == ROTATE_NONE)
            ...
            for (int i = 0;
                ...
                fCharStrings[i] = new String(data,
                                             i,
                                             1);
    ...
    public void paintIcon(final Component c,
        ...
        // Use Graphics2D to get better rendering
        ...
        graphics2D.setColor(c.getForeground());
        graphics2D.setFont(c.getFont());

        if (fRotation == ROTATE_NONE)
            {
            int yPos = y + fCharHeight;
            for (int i = 0;
                 i < fCharStrings.length;
                 i++)
                {
                ...
                switch (fPosition[i])
                    ...
                    case POSITION_NORMAL:
                        ...
                        graphics2D.drawString(fCharStrings[i],
                                     x + ((fWidth - fCharWidths[i]) >> 1),
                                     yPos);
                        break;
            ...
            {
            graphics2D.translate(x + fWidth,
                        y + fHeight);
            graphics2D.rotate(-NINETY_DEGREES);
            graphics2D.drawString(fLabel,
                         kBufferSpace,
                         -fDescent);
            graphics2D.rotate(NINETY_DEGREES);
            graphics2D.translate(-(x + fWidth),
    ...
    private static int verifyRotation(final String label,
                                     final int rotateHint)
        {
        ...
        if (hasMustRotate)
            {
            return DEFAULT_MUST_ROTATE;
            }
        else
            {
            return DEFAULT_ROMAN;
            }
        }
}