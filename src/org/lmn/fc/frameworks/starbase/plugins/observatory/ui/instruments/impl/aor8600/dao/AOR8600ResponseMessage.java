...
/***********************************************************************************************
 * AOR8600ResponseMessage.
 */

public final class AOR8600ResponseMessage extends AbstractResponseMessage
    {
    public static final int LENGTH_VALID_RESPONSE = 5;


    /***********************************************************************************************
     * Parse a byte array, attempting to find a AOR8600ResponseMessage
     * which can be interpreted as coming from a AOR8600 Instrument.
     *      reset()
     *      getRadiometerOutput()
    ...
    public synchronized static ResponseMessageInterface parseBytes(final CommandMessageInterface commandmessage,
                                                                   ...
                                                                   final byte[] bytes)
        {
        final String SOURCE = "AOR8600ResponseMessage.parseBytes() ";
        ResponseMessageInterface responseMessage;

//        LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() [" + Utilities.byteArrayToSpacedHex(bytes) + "]");
        ...
        // since all AOR8600 responses are four characters plus <cr>
        ...
        if ((commandmessage != null)
            ...
            if ((commandType != null)
                ...
                if ((bytes != null)
                    ...
                    // Here is where we cheat with prior knowledge of the AOR8600 data format...
                    ...
                    if (SchemaDataType.HEX_INTEGER.equals(commandType.getResponse().getDataTypeName()))
                        {
//                        LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() Received HexInteger [bytes="
...
//                        LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() Revised bytes to parse [bytes="
...
//                    LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() [ResponseValue="
//                                                + commandType.getResponse().getValue() + "] [errors=" + intParsingFailures + "]");
                    ...
                    LOGGER.error("AOR8600ResponseMessage.parseStaribusBytes() ResponseMessage appears malformed - a ResponseValue was expected, but was missing [status="
...
//                LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() No ResponseValue expected for this Command");
                }
            ...
            if (intParsingFailures == 0)
                ...
                responseMessage = new AOR8600ResponseMessage(dao.getPort().getName(),
...
//                LOGGER.debugProtocolEvent("AOR8600ResponseMessage.parseStaribusBytes() ResponseMessage successfully created");
                }
            else
                {
                // We couldn't parse anything, so can't provide a message to carry any Status
                LOGGER.error("AOR8600ResponseMessage.parseStaribusBytes() Failed to parse Response");
                }
            }
        else
            {
            // We couldn't parse anything, so can't provide a message to carry any Status
            LOGGER.error("AOR8600ResponseMessage.parseStaribusBytes() Invalid DAO or context");
            }
    ...
    /***********************************************************************************************
     * Build the byte array which represents the Response of the supplied context as AOR8600.
    ...
    private synchronized static List<Byte> buildByteArray(final Instrument instrumentxml,
        ...
        if (responseType != null)
            ...
            // A AOR8600 ResponseValue is just a String, to be added character by character
    ...
    /***********************************************************************************************
     * Construct a AOR8600ResponseMessage.
    ...
    public AOR8600ResponseMessage(final String portname,
