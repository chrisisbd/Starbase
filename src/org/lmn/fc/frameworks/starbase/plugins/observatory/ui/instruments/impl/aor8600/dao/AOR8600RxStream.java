...
public final class AOR8600RxStream extends AbstractRxStream
    ...
    private static final byte AOR8600_TERMINATOR = ControlCharacters.CR.getByteCode();
    ...
    /***********************************************************************************************
     * Construct a AOR8600RxStream.
     *
     * @param resourcekey
     */

    public AOR8600RxStream(final String resourcekey)
        {
        super(resourcekey);
    ...
    public boolean open() throws IOException
        ...
        LOGGER.debugTimedEvent(LOADER_PROPERTIES.isTimingDebug(),
                               "AOR8600RxStream.open()");

        readResources();

        // We rely on the AOR8600TxStream to set things up...
    ...
    public ResponseMessageInterface read(final InstrumentsDocument instrumentsdoc) throws IOException
        ...
        if ((readBuffer != null)
            ...
            LOGGER.debugCommandEvent(LOADER_PROPERTIES.isStaribusDebug(),
                                      "AOR8600RxStream.read() [bytes=" + Utilities.byteArrayToSpacedHex(readBuffer.toString().getBytes()) + "]");
            //LOGGER.debugProtocolEvent("AOR8600RxStream.read() [bytes=" + Utilities.byteArrayToExpandedAscii(readBuffer.toString().getBytes()) + "]");
            ...
            response = AOR8600ResponseMessage.parseBytes(getCommandContext(),
                                                         getHostPort().getHostDAOs(),
                                                         readBuffer.toString().getBytes());
//            if (response != null)
//                {
//                LOGGER.debugProtocolEvent("AOR8600RxStream.read() Message parsed, waiting for PortController to read");
//                }
//
//            if (response == null)
//                {
//                // TODO Review sending non-null response to avoid pseudo-timeout status
//                LOGGER.debugProtocolEvent("AOR8600RxStream.read() Message parsing failed, pseudo-TIMEOUT behaviour");
//                }
...
//                {
//                LOGGER.debugProtocolEvent("AOR8600RxStream.read() [ResponseValue=" + response.getCommandType().getResponse().getValue() + "]");
//                }
//            else if (response.getCommandType().getAck() != null)
//                {
//                LOGGER.debugProtocolEvent("AOR8600RxStream.read() [Ack]");
//                }
//            }
    ...
    private void setReadyToRead(final boolean ready)
        {
        //LOGGER.debugTimedEvent("AOR8600RxStream.setReadyToRead() [ready=" + ready + "]");
    ...
    public void readResources()
        {
        LOGGER.debugTimedEvent(LOADER_PROPERTIES.isTimingDebug(),
                               "AOR8600RxStream.readResources() [ResourceKey=" + getResourceKey() + "]");
        }
    ...
    public final void serialEvent(final SerialPortEvent event)
        ...
        switch(event.getEventType())
            ...
            case SerialPortEvent.DATA_AVAILABLE:
                {
                //LOGGER.debugProtocolEvent("AOR8600RxStream.serialEvent() SerialPortEvent.DATA_AVAILABLE");
                ...
                if (!isReadyToRead())
                    ...
                    try
                        ...
                        //LOGGER.debugTimedEvent("AOR8600RxStream.serialEvent() ENTER SERIAL WAIT LOOP [length=" + readBuffer.length() + "]");

                        // Allow enough time to read the longest message
                        intRetries = RETRY_COUNT;
                        boolFoundTerminator = false;

                        //LOGGER.debugTimedEvent("AOR8600RxStream.serialEvent() [retries=" + intRetries + "]");
                        ...
                        while ((intRetries > 0)
                            ...
                            while ((getUnderlyingRxStream().available() > 0)
                                ...
                                // Was this character the terminator?
                                if ((byte)intData == AOR8600_TERMINATOR)
                                    {
                                    boolFoundTerminator = true;

                                    // Leave all loops once the terminator is found
                                    setReadyToRead(boolFoundTerminator);
                                    }
                                }
                            ...
                            if (!boolFoundTerminator)
                                ...
                                //System.out.println("sleep ... buffer=[" + Utilities.byteArrayToSpacedHex(readBuffer.toString().getBytes()) + "]");
                        ...
                        // If we never found the terminator, the PortController will time out,
                        // because ReadyToRead will never be set...

                        //LOGGER.debugTimedEvent("AOR8600RxStream.serialEvent() LEAVE SERIAL WAIT LOOP [length=" + readBuffer.length() + "]");
//                        if (boolFoundTerminator)
//                            {
//                            LOGGER.debugProtocolEvent("AOR8600RxStream.serialEvent() ready to be read");
//                            }
                        }
                        ...
                        LOGGER.debugCommandEvent(LOADER_PROPERTIES.isStaribusDebug(),
                                                  "AOR8600RxStream.serialEvent() ERROR IOException reading data from input stream");
                        }
                    }
                else
                    {
                    // This should never happen...
                    //LOGGER.error("AOR8600RxStream.serialEvent() ERROR Waiting for PortController");
            ...
            case SerialPortEvent.OUTPUT_BUFFER_EMPTY:
                {
                LOGGER.error("AOR8600RxStream.serialEvent() ERROR Serial Event type [eventtype=" + event.getEventType() + "]");
                break;
                }
            ...
            default:
                ...
                LOGGER.error("AOR8600RxStream.serialEvent() ERROR Unknown Serial Event type [eventtype=" + event.getEventType() + "]");
                break;
                }
            }
        }
