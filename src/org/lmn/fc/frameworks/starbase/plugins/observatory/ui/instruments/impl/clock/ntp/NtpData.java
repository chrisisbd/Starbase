...
/**
 * This class represents a datastructure describing  the useful information that can be extracted
 * from a NtpDatagram returning from the server. We refer to rfc2030 for more details.
 */

public final class NtpData implements FrameworkConstants,
                                      FrameworkStrings
    {
    /**
    ...
    /**
     * Version number of the packet. In this application we always send version 3 packet to the
     * server. The servers always seem to reply with version 3 packets (and not version 4).
     */
    public int versionNumber;

    /**
     * Mode of the communication with the server. In our application this is MODE_CLIENT for the
     * client and MODE_SERVER for the server.
     */
    public int mode;

    /**
     * The stratum. This number indicates the distance (in hops) from the server to the primary
     * server (which is stratum 1).
     */
    public int stratum;
    ...
    /**
     * Reference Identifier. <UL> <LI> In the case of NTP Version 3 or Version 4 stratum-0
     * (unspecified) or stratum-1 (primary) servers, this is a String identifying the source. <LI>
     * In NTP Version 3 secondary servers, this is the InetAddress of the reference source. <LI>  In
     * NTP Version 4 secondary servers this is a 4-byte array. See rfc2030. </UL>
     */
     ...
     */
    public NtpTimeStamp referenceNtpTimeStamp;
    ...
    /**
     * Offset of the local clock versus the server clock, taking into account the roundtrip delay
     * (in milliseconds). Calculated according to rfc2030.
     */
    public long offset;
    ...
    public String toString()
        ...
        if (referenceIdentifier instanceof InetAddress)
            {
            s = s + "Reference address : " + referenceIdentifier + "\n";
            }
        else if (referenceIdentifier instanceof String)
            {
            s = s + "Reference code : " + referenceIdentifier + "\n";
            }
        else
            {
            final byte[] temp = (byte[]) referenceIdentifier;
            s = s + "Reference data : " + temp[0] + SPACE + temp[1] + SPACE + temp[2] + SPACE + temp[3] + "\n";
            }
        s = s + "Reference timestamp : " + referenceNtpTimeStamp + "\n" +
