...
public class FixedOscillator extends AudioInputStream
    ...
    public FixedOscillator(final AudioFormat audioformat,
        ...
        // Injections
//        LOGGER.logTimedEvent("FixedOscillator: Waveform=" + waveformType.getWaveformType());
//        LOGGER.logTimedEvent("FixedOscillator: Frequency=" + frequency);
//        LOGGER.logTimedEvent("FixedOscillator: Fractional Amplitude=" + amplitude);
//        LOGGER.logTimedEvent("FixedOscillator: Scaled Amplitude=" + dblScaledAmplitude);
//        LOGGER.logTimedEvent("FixedOscillator: Sampling Increment=" + dblSamplingIncrementRadians);
//        LOGGER.logTimedEvent("FixedOscillator: Frames per second=" + getFormat().getFrameRate());
//        LOGGER.logTimedEvent("FixedOscillator: Frame Rate/Frequency=" + (getFormat().getFrameRate() / dblFrequency));
//        LOGGER.logTimedEvent("FixedOscillator: Remaining Frames=" + longRemainingFrames);
//        LOGGER.logTimedEvent("FixedOscillator: Sample Word Size=" + dblSampleWordSize);
    ...
    public int read(final byte[] arraybytes,
        ...
        {
        final double HARMONIC_SCALING = 0.717;
...
//        LOGGER.logTimedEvent("FixedOscillator.read(): Requested Length=" + length);
//        LOGGER.logTimedEvent("FixedOscillator.read(): Destination Offset=" + intDestinationOffset);

//        LOGGER.logTimedEvent("FixedOscillator.read(): Frames To Deliver=" + intFramesToBeDelivered);
//        LOGGER.logTimedEvent("FixedOscillator.read(): Frames Remaining=" + longRemainingFrames);
//        LOGGER.logTimedEvent("FixedOscillator.read(): Phase Radians=" + dblPhaseRadians);
        ...
        for (int intFrameIndex = 0;
            ...
            switch (getWaveformType())
                ...
                case HARMONIC_SERIES:
                    {
                    dblValueForFrame = Math.sin(dblPhaseRadians) / 5.0
                                        + Math.sin(dblPhaseRadians * 2.0) / 5.0
                                        + Math.sin(dblPhaseRadians * 3.0) / 5.0
                                        + Math.sin(dblPhaseRadians * 4.0) / 5.0
                                        + Math.sin(dblPhaseRadians * 5.0) / 5.0;

                    // Lose 6dB on each harmonic
//                    dblValueForFrame = (Math.sin(dblPhaseRadians)
//                                        + Math.sin(dblPhaseRadians * 2.0) / 2.0
//                                        + Math.sin(dblPhaseRadians * 3.0) / 4.0
//                                        + Math.sin(dblPhaseRadians * 4.0) / 8.0
//                                        + Math.sin(dblPhaseRadians * 5.0) / 16.0) * HARMONIC_SCALING;
//
                    if (Math.abs(dblValueForFrame) > 1.0)
                        {
                        LOGGER.error("FixedOscillator.read() [error=Out of range] [value= " + dblValueForFrame + "] [phase=" + dblPhaseRadians + "]");
                        }
                    break;
                    }
            ...
            if (intValueForFrame > dblSampleWordSize)
                ...
                //LOGGER.error("FixedOscillator.read() ERROR! Value out of range!");
            ...
            switch (getFormat().getChannels())
                ...
                case STEREO:
                    ...
                    if (getFormat().isBigEndian())
                        ...
                        switch (getFormat().getSampleSizeInBits())
                            ...
                            default:
                                {
                                LOGGER.error("FixedOscillator.read() ERROR! Big Endian Sample Size in Bits not supported!");
                                }
                            }
                        }
                    else
                        ...
                        switch (getFormat().getSampleSizeInBits())
                            ...
                            default:
                                {
                                LOGGER.error("FixedOscillator.read() ERROR! Little Endian Sample Size in Bits not supported!");
                                }
                            }
                        }
                    break;
                ...
                default:
                    {
                    LOGGER.error("FixedOscillator.read() ERROR! Channel Count not supported!");
                    }
                }
            }
    ...
    public int read() throws IOException
        {
        throw new IOException("FixedOscillator.read() Method not implemented");
        }
    }
