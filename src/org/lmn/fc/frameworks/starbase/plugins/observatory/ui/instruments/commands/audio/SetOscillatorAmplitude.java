...
public final class SetOscillatorAmplitude implements FrameworkConstants,
                                                     FrameworkStrings,
                                                     FrameworkMetadata,
                                                     FrameworkSingletons,
    ...
    public static ResponseMessageInterface doSetOscillatorAmplitude(final ObservatoryInstrumentDAOInterface dao,
                                                                    final OscillatorInterface oscillator,
                                                                    final CommandMessageInterface commandmessage)
        {
        final String SOURCE = "SetOscillatorAmplitude.doSetOscillatorAmplitude() ";
        final int PARAMETER_COUNT = 1;
        final int INDEX_AMPLITUDE = 0;
        final List<ParameterType> listParameters;
        final CommandType cmdSetAmplitude;
        String strResponseValue;
        final ResponseMessageInterface responseMessage;

        // Don't affect the CommandType of the incoming Command
        cmdSetAmplitude = (CommandType)commandmessage.getCommandType().copy();

        // Parameters
        listParameters = cmdSetAmplitude.getParameterList();
        ...
        if ((dao.getHostInstrument() != null)
            ...
            try
                {
                final String strAmplitude;
                final double dblParamAmplitude;

                strAmplitude = listParameters.get(INDEX_AMPLITUDE).getValue();
                dblParamAmplitude = Double.parseDouble(strAmplitude);

                oscillator.setSignalAmplitude(dblParamAmplitude);
            ...
            // This should have been trapped by Regex
                ...
                SimpleEventLogUIComponent.logEvent(dao.getEventLogFragment(),
                                                   ...
                                                   METADATA_TARGET + OscillatorInterface.TARGET_OSCILLATOR + TERMINATOR
                                                       + METADATA_ACTION_SET
                                                       + METADATA_ITEM + listParameters.get(INDEX_AMPLITUDE).getName() + TERMINATOR_SPACE
                                                       + METADATA_RESULT
