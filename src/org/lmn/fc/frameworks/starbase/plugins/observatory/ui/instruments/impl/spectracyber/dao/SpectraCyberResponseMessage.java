...
/***********************************************************************************************
 * SpectraCyberResponseMessage.
 */

public final class SpectraCyberResponseMessage extends AbstractResponseMessage
    {
    public static final int LENGTH_VALID_RESPONSE = 5;


    /***********************************************************************************************
     * Parse a byte array, attempting to find a SpectraCyberResponseMessage
    ...
    public synchronized static ResponseMessageInterface parseBytes(final CommandMessageInterface commandmessage,
                                                                   ...
                                                                   final byte[] bytes)
        {
        final String SOURCE = "SpectraCyberResponseMessage.parseBytes() ";
        ResponseMessageInterface responseMessage;

//        LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() [" + Utilities.byteArrayToSpacedHex(bytes) + "]");
            ...
            if ((commandType != null)
                ...
                if ((bytes != null)
                    ...
                    if (SchemaDataType.HEX_INTEGER.equals(commandType.getResponse().getDataTypeName()))
                        {
//                        LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() Received HexInteger [bytes="
...
//                        LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() Revised bytes to parse [bytes="
...
//                    LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() [ResponseValue="
//                                                + commandType.getResponse().getValue() + "] [errors=" + intParsingFailures + "]");
                    ...
                    LOGGER.error("SpectraCyberResponseMessage.parseStaribusBytes() ResponseMessage appears malformed - a ResponseValue was expected, but was missing [status="
...
//                LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() No ResponseValue expected for this Command");
                }
            ...
            if (intParsingFailures == 0)
                ...
                responseMessage = new SpectraCyberResponseMessage(dao.getPort().getName(),
...
//                LOGGER.debugProtocolEvent("SpectraCyberResponseMessage.parseStaribusBytes() ResponseMessage successfully created");
                }
            else
                {
                // We couldn't parse anything, so can't provide a message to carry any Status
                LOGGER.error("SpectraCyberResponseMessage.parseStaribusBytes() Failed to parse Response");
                }
            }
        else
            {
            // We couldn't parse anything, so can't provide a message to carry any Status
            LOGGER.error("SpectraCyberResponseMessage.parseStaribusBytes() Invalid DAO or context");
            }
    ...
    /***********************************************************************************************
     * Construct a SpectraCyberResponseMessage.
    ...
    public SpectraCyberResponseMessage(final String portname,
