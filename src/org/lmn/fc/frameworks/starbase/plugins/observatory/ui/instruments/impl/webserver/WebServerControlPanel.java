...
/***************************************************************************************************
 * The WebServerControlPanel.
 */

public final class WebServerControlPanel extends InstrumentUIComponentDecorator
                                         implements ControlPanelInterface
    {
    // String Resources
    private static final String ICON_CONTROL = "webserver-control.png";
    private static final String KEY_WEBSERVER_PORT = "Webserver.Port";
    private static final String TOOLTIP_PORT = "The port on which the Webserver is listening";
    ...
    /***********************************************************************************************
     * Construct the WebServerControlPanel.
     *
     * @param instrument
     * @param oui
     * @param instrumentxml
    ...
    public WebServerControlPanel(final ObservatoryInstrumentInterface instrument,
        ...
        this.intServerPort = WebServer.DEFAULT_PORT;
        }
    ...
    public void instrumentChanged(final InstrumentStateChangedEvent event)
        {
        final String SOURCE = "WebServerControlPanel.instrumentChanged() ";
        final int intDisplayPort;

        LOGGER.debugIndicators(SOURCE + "--> update indicators");

        super.instrumentChanged(event);

        // Create or Update the Port Metadata item every time the Indicator is updated
        if ((getHostInstrument() != null)
            && (getHostInstrument().getAggregateMetadata() != null))
            {
            final Metadata metadataItem;

            // Find the Webserver Port
            if (intServerPort != 0)
                {
                if ((getHostInstrument() instanceof WebServer)
                    && (((WebServer) getHostInstrument()).getJettyServer() != null)
                    && (((WebServer) getHostInstrument()).getJettyServer().isRunning()))
                    {
                    intDisplayPort = intServerPort;
                    }
                else
                    {
                    intDisplayPort = 0;
            ...
            metadataItem = MetadataHelper.createMetadata(KEY_WEBSERVER_PORT,
                                                         Integer.toString(intDisplayPort),
                                                         REGEX_NONE,
                                                         DataTypeDictionary.DECIMAL_INTEGER,
                                                         SchemaUnits.DIMENSIONLESS,
                                                         TOOLTIP_PORT);
            MetadataHelper.addOrUpdateMetadataItemTraced(getHostInstrument().getAggregateMetadata(),
                                                         metadataItem,
                                                         SOURCE,
    ...
    /* Utilities                                                                                  */
    /***********************************************************************************************
     *  Read all the Resources required by the WebServerControlPanel.
     */

    public void readResources()
        {
        LOGGER.debugTimedEvent(LOADER_PROPERTIES.isTimingDebug(),
                               "WebServerControlPanel [ResourceKey=" + getResourceKey() + "]");

        intServerPort = REGISTRY.getIntegerProperty(getResourceKey() + KEY_PORT);
        }
    }
