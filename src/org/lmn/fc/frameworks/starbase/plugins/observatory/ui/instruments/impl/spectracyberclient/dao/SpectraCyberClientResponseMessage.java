...
/***********************************************************************************************
 * SpectraCyberClientResponseMessage.
 */

public final class SpectraCyberClientResponseMessage extends AbstractResponseMessage
    {
    public static final int LENGTH_VALID_RESPONSE = 5;


    /***********************************************************************************************
     * Parse a byte array, attempting to find a SpectraCyberClientResponseMessage
     * which can be interpreted as coming from a SpectraCyber Instrument.
     *      reset()
     *      getRadiometerOutput()
    ...
    public synchronized static ResponseMessageInterface parseSpectraCyberBytes(final CommandMessageInterface commandmessage,
                                                                               final List<ObservatoryInstrumentDAOInterface> daos,
                                                                               final byte[] bytes)
        {
        final String SOURCE = "SpectraCyberClientResponseMessage.parseSpectraCyberBytes() ";
        ResponseMessageInterface responseMessage;

//        LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseSpectraCyberBytes() [" + Utilities.byteArrayToSpacedHex(bytes) + "]");
        ...
        // since all SpectraCyber responses are four characters plus <cr>
        ...
        if ((commandmessage != null)
            ...
            if ((commandType != null)
                ...
                if ((bytes != null)
                    ...
                    // Here is where we cheat with prior knowledge of the SpectraCyber data format...
                    ...
                    if (SchemaDataType.HEX_INTEGER.equals(commandType.getResponse().getDataTypeName()))
                        {
//                        LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseStaribusBytes() Received HexInteger [bytes="
...
//                        LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseStaribusBytes() Revised bytes to parse [bytes="
...
//                    LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseStaribusBytes() [ResponseValue="
//                                                + commandType.getResponse().getValue() + "] [errors=" + intParsingFailures + "]");
                    ...
                    LOGGER.error("SpectraCyberClientResponseMessage.parseStaribusBytes() ResponseMessage appears malformed - a ResponseValue was expected, but was missing [status="
...
//                LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseStaribusBytes() No ResponseValue expected for this Command");
                }
            ...
            if (intParsingFailures == 0)
                ...
                responseMessage = new SpectraCyberClientResponseMessage(dao.getPort().getName(),
...
//                LOGGER.debugProtocolEvent("SpectraCyberClientResponseMessage.parseStaribusBytes() ResponseMessage successfully created");
                }
            else
                {
                // We couldn't parse anything, so can't provide a message to carry any Status
                LOGGER.error("SpectraCyberClientResponseMessage.parseStaribusBytes() Failed to parse Response");
                }
            }
        else
            {
            // We couldn't parse anything, so can't provide a message to carry any Status
            LOGGER.error("SpectraCyberClientResponseMessage.parseStaribusBytes() Invalid DAO or context");
            }
    ...
    /***********************************************************************************************
     * Build the byte array which represents the Response of the supplied context as SpectraCyber.
    ...
    private synchronized static List<Byte> buildByteArray(final Instrument instrumentxml,
        ...
        if (responseType != null)
            ...
            // A SpectraCyber ResponseValue is just a String, to be added character by character
    ...
    /***********************************************************************************************
     * Construct a SpectraCyberClientResponseMessage.
    ...
    public SpectraCyberClientResponseMessage(final String portname,
