...
/***************************************************************************************************
 * A SimpleEventLogUIComponent.
 */

public final class SimpleEventLogUIComponent extends ReportTable
                                             implements ReportTablePlugin
    {
    // String Resources
    private static final String HEADER_TITLE = "Event Log";
    ...
    /***********************************************************************************************
     * Log an Event from the session on a SimpleEventLogUIComponent.
     *
     * @param log
     * @param status
     * @param event
    ...
    public static void logEvent(final Vector<Vector> log,
        ...
        LOGGER.debugTimedEvent(true, event);

        vecLogFragment = createEventLogFragment(status, event, source, clock);
        ...
        if ((log != null)
            ...
            {
            LOGGER.error("SimpleEventLogUIComponent.logEvent() Can't add to log");
            }
        }


    /***********************************************************************************************
     * Log a List of Errors from the session on a SimpleEventLogUIComponent.
     * Note that these may be verbose, and not follow the usual syntax [target} [action] etc.
     *
     * @param log
     * @param status
     * @param errors
    ...
    public static void logErrors(final Vector<Vector> log,
                                 final EventStatus status,
        ...
        if ((errors != null)
            ...
            while (iterErrors.hasNext())
                ...
                strError = iterErrors.next();
                LOGGER.debugTimedEvent(true, strError);

                vecLogFragment = createEventLogFragment(status, strError, source, clock);
    ...
    /***********************************************************************************************
     * Create an EventLogFragment from the specified values.
    ...
    private static Vector<Object> createEventLogFragment(final EventStatus status,
                                                         final String event,
                                                         final String source,
                                                         final ObservatoryClockInterface clock)
        {
        ...
        if ((status != null)
            ...
            //LOGGER.debugProtocolEvent("SimpleEventLogUIComponent.createEventLogFragment() [event=" + event + "]");
            ...
            {
            LOGGER.error("SimpleEventLogUIComponent.createEventLogFragment() Could not create a log entry");
            }
    ...
    /***********************************************************************************************
     * Construct an SimpleEventLogUIComponent for the specified Task.
    ...
    public SimpleEventLogUIComponent(final TaskPlugin task,
        ...
        super(task,
              ...
              RegistryModelUtilities.getCommonIcon(EVENT_LOG_ICON_FILENAME));
    ...
    public final Vector<String> generateHeader()
        ...
        vecHeader.add("This report shows all Command Events related to this Instrument");
    ...
    /***********************************************************************************************
     * Generate the SimpleEventLogUIComponent table.
    ...
    public final Vector<Vector> generateReport()
        {
        final String SOURCE = "SimpleEventLogUIComponent.generateReport() ";
        ...
        if ((UIComponentHelper.shouldRefresh(false, getHostInstrument(), this))
            ...
            && (getDataViewLimit() > 0))
            {
            // Make a copy of the Log for truncation and sorting...
            vecReport = (Vector<Vector>) getEventLog().clone();

            // All EventLogs are always sorted in descending order
            // Sort the copy of the Log by the 'hidden' last Integer column
            // See AbstractObservatoryInstrument.addEventLogFragment()
    ...
    public Vector<Vector> generateRawReport() throws ReportException
        ...
        // Sort the copy of the Log by the 'hidden' last Integer column
        // All EventLogs are sorted in descending order
        Collections.sort(vecReport,
                         new ReportRowsByColumn(LogHelper.DEFAULT_LOG_COLUMN_COUNT));
    ...
    /***********************************************************************************************
     * Refresh the SimpleEventLogUIComponent data table.
     *
     * @return Vector
     */
    ...
    public synchronized final void truncateReport()
        ...
        if ((getEventLog() != null)
            && (getEventLog().size() > LOGSIZE_TRUNCATE))
            {
    ...
    /***********************************************************************************************
     * Read all the Resources required by the SimpleEventLogUIComponent.
     * The ResourceKey is always that of the host Framework, since this is a general utility.
     */
    ...
    /***********************************************************************************************
     * Get the EventLog from the Instrument.
     *
     * @return Vector<Vector>
     */

    private synchronized Vector<Vector> getEventLog()
        {
        return (getHostInstrument().getEventLog());
        }
