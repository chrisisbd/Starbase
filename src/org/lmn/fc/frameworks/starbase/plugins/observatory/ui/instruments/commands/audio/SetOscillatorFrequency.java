...
public final class SetOscillatorFrequency implements FrameworkConstants,
                                                     FrameworkStrings,
                                                     FrameworkMetadata,
                                                     FrameworkSingletons,
    ...
    public static ResponseMessageInterface doSetOscillatorFrequency(final ObservatoryInstrumentDAOInterface dao,
                                                                    final OscillatorInterface oscillator,
                                                                    final CommandMessageInterface commandmessage)
        {
        final String SOURCE = "SetOscillatorFrequency.doSetOscillatorFrequency() ";
        final int PARAMETER_COUNT = 1;
        final int INDEX_FREQUENCY = 0;
        final List<ParameterType> listParameters;
        final CommandType cmdSetFreq;
        String strResponseValue;
        final ResponseMessageInterface responseMessage;

        // Don't affect the CommandType of the incoming Command
        cmdSetFreq = (CommandType)commandmessage.getCommandType().copy();

        // Parameters
        listParameters = cmdSetFreq.getParameterList();

        LOGGER.debug(LOADER_PROPERTIES.isThreadsDebug(),
                     SOURCE + "Go to check parameters");
        ...
        if ((dao.getHostInstrument() != null)
            ...
            && (listParameters.get(INDEX_FREQUENCY) != null)
            && (SchemaDataType.DECIMAL_FLOAT.equals(listParameters.get(INDEX_FREQUENCY).getInputDataType().getDataTypeName())))
            {
            try
                {
                final String strFrequency;
                final double dblFrequency;

                strFrequency = listParameters.get(INDEX_FREQUENCY).getValue();
                dblFrequency = Double.parseDouble(strFrequency);

                LOGGER.debug(LOADER_PROPERTIES.isThreadsDebug(),
                             SOURCE + "Go to oscillator.setSignalFrequencyStart(" + dblFrequency + ")");

                oscillator.setSignalFrequencyStart(dblFrequency);
            ...
            // This should have been trapped by Regex
                ...
                SimpleEventLogUIComponent.logEvent(dao.getEventLogFragment(),
                                                   ...
                                                   METADATA_TARGET + OscillatorInterface.TARGET_OSCILLATOR + TERMINATOR
                                                       + METADATA_ACTION_SET
                                                       + METADATA_ITEM + listParameters.get(INDEX_FREQUENCY).getName() + TERMINATOR_SPACE
                                                       + METADATA_RESULT
