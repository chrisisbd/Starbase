...
public final class ExportEventLog implements FrameworkConstants,
                                             FrameworkStrings,
                                             FrameworkMetadata,
                                             FrameworkSingletons,
    ...
    public static ResponseMessageInterface doExportEventLog(final ObservatoryInstrumentDAOInterface dao,
                                                            final CommandMessageInterface commandmessage)
        {
        final String SOURCE = "ExportEventLog.exportEventLog()";
        ...
        if ((dao.getWrappedData() != null)
            ...
            try
                ...
                if (translator != null)
                    ...
                    // The EventLog comes directly from the host Instrument
                    // Note that it will not contain the events from this invocation of exportLog()
                    // Remember that the Log presented by the Instrument
                    // contains the extra last index column used for sorting, not needed by the exporter
                    boolSuccess = dao.getTranslator().exportLog(dao.getHostInstrument().getEventLogMetadata(),
                                                                dao.getHostInstrument().getEventLog(),
                                                                dao.getHostInstrument().getEventLogWidth(),
                                                                strFilename,
                                                                boolTimestamp,
