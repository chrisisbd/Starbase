...
public class StaribusBlockParser implements DataTypeParserInterface
     {
     ...
     public static DataTypeParserInterface getInstance()
         ...
         if (PARSER_INSTANCE == null)
             {
             synchronized (StaribusBlockParser.class)
                 {
                 if (PARSER_INSTANCE == null)
                     {
                     PARSER_INSTANCE = new StaribusBlockParser();
                     }
                 }
             }
     ...
     /***********************************************************************************************
      * Privately construct the StaribusBlockParser.
      */

     private StaribusBlockParser()
         {
         super();
         }
     ...
     public boolean parseBytesToResponseValue(final byte[] bytes,
         ...
         if ((responsetype.getRegex() != null)
             ...
             if (Pattern.matches(responsetype.getRegex(), valuefield))
                 ...
                 {
                 errors.add("[name=" + responsetype.getName() + "]" + INVALID_STARIBUS_BLOCK);
             ...
             // Do the best we can
             // All we have to do is to check that the block is of the correct length,
             // and contains only printable ASCII characters
             if ((valuefield.length() == StaribusParsers.LENGTH_STARIBUS_BLOCK)
                 && (Pattern.matches(REGEX_STARIBUS_BLOCK, valuefield)))
                 {
                 // Accept the Value
                 }
             else
