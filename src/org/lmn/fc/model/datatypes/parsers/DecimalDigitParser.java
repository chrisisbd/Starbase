...
public class DecimalDigitParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (DecimalDigitParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new DecimalDigitParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the DecimalDigitParser.
     */

    private DecimalDigitParser()
        {
        super();
        }
    ...
    public boolean validateParameterValueAsDataType(final ParameterType parameter,
        ...
        if ((parameter.getRegex() != null)
            ...
            if (Pattern.matches(parameter.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_DECIMAL_DIGIT);
            ...
            // Do the best we can
            if (Pattern.matches(FrameworkRegex.REGEX_DECIMAL_DIGIT, valuefield))
                {
                // Accept the Value
                }
            else
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_DECIMAL_DIGIT);
                errors.add("[name=" + parameter.getName() + "]" + MSG_REGEX + REGEX_DECIMAL_DIGIT);
                intFailures++;
                }
            }
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_DECIMAL_DIGIT);
            ...
            // ToDo Consider using Regex from the DataType
            // Do the best we can
            if (Pattern.matches(REGEX_DECIMAL_DIGIT, valuefield))
                {
                // Accept the Value
                }
            else
                {
                errors.add("[" + valuefield + "]" + INVALID_DECIMAL_DIGIT);
                errors.add("[" + valuefield + "]" + MSG_REGEX + REGEX_DECIMAL_DIGIT);
                intFailures++;
                }
            }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            if ((valuefield != null)
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_DECIMAL_DIGIT);
