...
/***************************************************************************************************
 * StaribusMultichannelDataParser.
 */

public class StaribusMultichannelDataParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (StaribusMultichannelDataParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new StaribusMultichannelDataParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the StaribusMultichannelDataParser.
     */

    private StaribusMultichannelDataParser()
        {
        super();
        }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            {
            // Validate using Regex, but don't try to parse
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_SYNTAX);
            ...
            // Do the best we can
            // All we have to do is to check that the String contains only printable ASCII characters
            if ((Pattern.matches(REGEX_STARIBUS_MULTICHANNEL_DATA, valuefield)))
                {
                // Accept the Value
                }
            else
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_STARIBUS_MULTICHANNEL_DATA);
                intFailures++;
                }
            }
