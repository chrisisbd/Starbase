...
public class BitStringParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (BitStringParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new BitStringParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the BitStringParser.
     */

    private BitStringParser()
        {
        super();
        }
    ...
    public boolean validateParameterValueAsDataType(final ParameterType parameter,
        ...
        if ((parameter.getRegex() != null)
            ...
            if (Pattern.matches(parameter.getRegex(), parameter.getValue()))
                ...
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_BIT_STRING);
            ...
            {
            // Do the best we can with the DataType Regex
            if (Pattern.matches(datatype.getRegex(), parameter.getValue()))
                {
                // Accept the Value
                }
            else
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_BIT_STRING);
                errors.add("[name=" + parameter.getName() + "]" + MSG_REGEX + datatype.getRegex());
                intFailures++;
                }
            }
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_BIT_STRING);
            ...
            {
            // Do the best we can with our own Regex
            if (Pattern.matches(REGEX_BIT_STRING, valuefield))
                {
                // Accept the Value
                }
            else
                {
                errors.add("[" + valuefield + "]" + INVALID_BIT_STRING);
                errors.add("[" + valuefield + "]" + MSG_REGEX + REGEX_BIT_STRING);
                intFailures++;
                }
            }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_BIT_STRING);
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_BIT_STRING);
                errors.add("[name=" + responsetype.getName() + "]" + MSG_REGEX + datatype.getRegex());
                intFailures++;
                }
            }
