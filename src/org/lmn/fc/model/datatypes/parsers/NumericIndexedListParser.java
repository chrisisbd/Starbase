...
/***************************************************************************************************
 * NumericIndexedListParser.
 */

public class NumericIndexedListParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (NumericIndexedListParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new NumericIndexedListParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the NumericIndexedListParser.
     */

    private NumericIndexedListParser()
        {
        super();
        }
    ...
    public boolean validateParameterValueAsDataType(final ParameterType parameter,
        ...
        if ((parameter.getRegex() != null)
            ...
            if (Pattern.matches(parameter.getRegex(), parameter.getValue()))
                {
                // The item appears in the Regex list
                // How can a String not be a String?!
                }
            else
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_NUMERIC_INDEXED_LIST);
            ...
            // Do the best we can - only one item in the list?
            // How can a String not be a String?!
            }
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_NUMERIC_INDEXED_LIST);
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_NUMERIC_INDEXED_LIST);
                errors.add("[" + valuefield + "]" + MSG_REGEX + responsetype.getRegex());
                intFailures++;
                }
            }
        else
