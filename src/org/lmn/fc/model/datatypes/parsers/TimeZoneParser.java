...
public class TimeZoneParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (TimeZoneParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new TimeZoneParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the TimeZoneParser.
     */
    ...
    public boolean validateParameterValueAsDataType(final ParameterType parameter,
        ...
        if ((parameter.getRegex() != null)
            ...
            if (Pattern.matches(parameter.getRegex(), parameter.getValue()))
                {
                // How can a TimeZone String not be a TimeZone?!
                // Just leave it alone
                }
            else
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_TIME_ZONE);
            ...
            if ((parameter.getValue() == null)
                ...
                {
                errors.add(parameter.getValue() + INVALID_TIME_ZONE);
                intFailures++;
                }
            else
                {
                // Validate using our own Regex
                if (Pattern.matches(FrameworkRegex.REGEX_TIMEZONE, parameter.getValue()))
                    {
                    // How can a TimeZone String not be a TimeZone?!
                    // Just leave it alone
                    }
                    ...
                    {
                    errors.add("[name=" + parameter.getName() + "]" + INVALID_TIME_ZONE);
                    errors.add("[name=" + parameter.getName() + "]" + MSG_REGEX + FrameworkRegex.REGEX_TIMEZONE);
                    intFailures++;
                    }
                }
            }
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_TIME_ZONE);
            ...
            {
            // Do the best we can with no Regex from the DataType
            if ((valuefield == null)
                ...
                {
                // Validate using our own Regex
                if (Pattern.matches(FrameworkRegex.REGEX_TIMEZONE, valuefield))
                    {
                    // How can a TimeZone String not be a TimeZone?!
                    // Just leave it alone
                    }
                    ...
                    {
                    errors.add("[" + valuefield + "]" + INVALID_TIME_ZONE);
                    errors.add("[" + valuefield + "]" + MSG_REGEX + FrameworkRegex.REGEX_TIMEZONE);
                    intFailures++;
                    }
                }
            }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_TIME_ZONE);
            ...
            if ((EMPTY_STRING.equals(valuefield)))
                ...
                if (Pattern.matches(FrameworkRegex.REGEX_TIMEZONE, valuefield))
                    ...
                    {
                    errors.add("[name=" + responsetype.getName() + "]" + INVALID_TIME_ZONE);
                    errors.add("[name=" + responsetype.getName() + "]" + MSG_REGEX + FrameworkRegex.REGEX_TIMEZONE);
                    intFailures++;
                    }
                }
            }
