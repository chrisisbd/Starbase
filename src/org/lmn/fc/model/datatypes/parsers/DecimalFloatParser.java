...
public class DecimalFloatParser implements DataTypeParserInterface
    {
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (DecimalFloatParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new DecimalFloatParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the DecimalFloatParser.
     */

    private DecimalFloatParser()
        {
        super();
        }
            ...
            if (Pattern.matches(parameter.getRegex(), valuefield))
                ...
                try
                    ...
                    if ((valuefield.length() > 1)
                        ...
                        {
                        Float.parseFloat(valuefield.substring(1));
                        }
                    else
                        {
                        Float.parseFloat(valuefield);
                        }
                    }
                ...
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_DECIMAL_FLOAT);
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                try
                    {
                    // Remove any leading '+' because of the dumb Float parser...
                    if ((valuefield.length() > 1)
                        && (valuefield.startsWith(PREFIX_PLUS)))
                        {
                        Float.parseFloat(valuefield.substring(1));
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_DECIMAL_FLOAT);
    ...
    public Number parseValueFieldToNumber(final String valuefield,
                                          ...
                                          final List<String> errors)
        {
        final String SOURCE = "DecimalFloat.parseValueFieldToNumber() ";
        Number number;
                ...
                if (Pattern.matches(datatype.getRegex(), valuefield))
                    ...
                    try
                        ...
                        if ((valuefield.length() > 1)
                            && (valuefield.startsWith(PREFIX_PLUS)))
                            {
                            number = Float.parseFloat(valuefield.substring(1));
                            }
                        else
                            {
                            number = Float.parseFloat(valuefield);
                            }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            {
            // Validate using Regex
            // The incoming Value is plain text, of ASCII codes of decimal numbers
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_DECIMAL_FLOAT);
