...
/***************************************************************************************************
 * UnsignedHexByteParser.
 */

public class UnsignedHexByteParser implements DataTypeParserInterface
    {
    // The Parser is a Singleton!
    private volatile static DataTypeParserInterface PARSER_INSTANCE;

    private final int UNSIGNED_MAX_VALUE = 255;
    ...
    public static DataTypeParserInterface getInstance()
        ...
        if (PARSER_INSTANCE == null)
            {
            synchronized (UnsignedHexByteParser.class)
                {
                if (PARSER_INSTANCE == null)
                    {
                    PARSER_INSTANCE = new UnsignedHexByteParser();
                    }
                }
            }
    ...
    /***********************************************************************************************
     * Privately construct the UnsignedHexByteParser.
     */

    private UnsignedHexByteParser()
        {
        super();
        }
            ...
            if (Pattern.matches(parameter.getRegex(), valuefield))
                ...
                try
                    ...
                    if ((valuefield.length() > 1)
                        ...
                        {
                        intValue = Integer.parseInt(valuefield.substring(1), datatype.getRadix());
                        }
                    else
                        {
                        intValue = Integer.parseInt(valuefield, datatype.getRadix());
                        }

                    // Unsigned values must not be negative!
                    if ((intValue < 0)
                        || (intValue > UNSIGNED_MAX_VALUE))
                        {
                        throw new NumberFormatException("Byte out of range [value=" + valuefield + "]");
                        }
                    }
                ...
                {
                errors.add("[name=" + parameter.getName() + "]" + INVALID_HEX_BYTE);
    ...
    public boolean validateValueFieldAsDataType(final String valuefield,
        ...
        if ((datatype.getRegex() != null)
            ...
            if (Pattern.matches(datatype.getRegex(), valuefield))
                ...
                {
                errors.add("[" + valuefield + "]" + INVALID_HEX_BYTE);
                ...
                if (Pattern.matches(datatype.getRegex(), valuefield))
                    ...
                    try
                        ...
                        if ((number.intValue() < 0)
                            || (number.intValue() > UNSIGNED_MAX_VALUE))
                            {
                            throw new NumberFormatException("Byte out of range [value=" + valuefield + "]");
                            }
                        }
    ...
    public boolean parseBytesToResponseValue(final byte[] bytes,
        ...
        if ((responsetype.getRegex() != null)
            ...
            {
            // Validate using Regex
            // The Response bytes are ASCII characters of a HEX representation of the signed Byte
            // Leading zeroes may be present when required
            if (Pattern.matches(responsetype.getRegex(), valuefield))
                ...
                {
                errors.add("[name=" + responsetype.getName() + "]" + INVALID_HEX_BYTE);
